#pragma once
// creds to conspiracy :D
namespace cloudsdk {
	namespace mem_internal {
		template <typename T = uintptr_t>

		static T rva_to_va(uintptr_t address, int size) {
			return address ? (T)(address + *(int*)(address + size - 4) + size) : T();
		}
	...
	static T find_pattern(const std::string& signature, int rva = 0)
	{
		...
		const auto dos_header = (PIMAGE_DOS_HEADER)GetModuleHandleA(0);
		const auto nt_headers = (PIMAGE_NT_HEADERS)((std::uint8_t*)GetModuleHandleA(0) + dos_header->e_lfanew);

		const auto module_size = nt_headers->OptionalHeader.SizeOfImage;
		const auto module_data = reinterpret_cast<std::uint8_t*>(GetModuleHandleA(0));
		...
		while (base_address < end_address) {
			...
			if (mem_internal::is_valid_region_ptr((void*)base_address, &region_size) && region_size) {
				uintptr_t region_end = base_address + region_size;
				for (auto i = base_address; i < region_end - size; ++i) {

					bool found = true;
					for (auto j = 0ul; j < size; ++j) {
						if (module_data[i - (uintptr_t)GetModuleHandleA(0) + j] != data[j] && data[j] != -1) {
							found = false;
							break;
						}
					}
					...
					if (found)
						return rva ? mem_internal::rva_to_va<T>(i, rva) : (T)i;
				}
			}
